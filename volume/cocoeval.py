...
class COCOeval:
    ...
    def evaluate(self):
        ...
        if p.iouType == 'bbox3d':
            computeIoU = self.computeIoU3D
        if p.iouType == 'segm' or p.iouType == 'bbox':
            raise NotImplementedError(NOT_IMPLEMENTED_NAPHAT)
        elif p.iouType == 'keypoints':
            computeIoU = self.computeOks

        tqdmkwargs = {
            'desc':'Calculating IoU',
            'ncols':70,
            'ascii':True
        }

        self.ious = {(imgId, catId): computeIoU(imgId, catId) \
                        for imgId in tqdm(p.imgIds, **tqdmkwargs)
                        for catId in catIds}
    ...
    def computeIoU3D(self, imgId, catId):
        ...
        for j, label in enumerate(gts):
            for i, output in enumerate(dts):
                ...
                outputBox = Box(outputBox[:3], outputBox[3:6], outputBox[6:9])
                ious[i,j] = iou3D(labelBox, outputBox)
    ...
    def summarize(self):
        ...
        if iouType in ['segm', 'bbox', 'bbox3d']:
